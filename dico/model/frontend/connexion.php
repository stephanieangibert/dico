...
function nouveauMdp($mdp1,$email){
 ...
 function  recupererMot($mot,$definition,$user_id){
     $db = dbConnect();
     $insertmot = $db->prepare("INSERT INTO mots(mot,def,user_id) VALUES(?, ?,?)");
     $insertmot->execute(array($mot,$definition,$user_id));  
     return  $insertmot; 
 }
 function mesMotsaMoi($id){
    $db = dbConnect();
    $req=$db->prepare("SELECT* FROM users INNER JOIN mots ON users.id=mots.user_id WHERE users.id =?");
